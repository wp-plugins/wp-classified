...
$pageinfo = $wpClassified->get_pageinfo();
$rssLink = get_bloginfo('wpurl'). "/?page_id=". $pageinfo["ID"]. "&_action=wpcfeed";
$atomLink= $rssLink;
...
<?php ob_start(); start_wp();?>
	<item>
		<title><?php echo wpcRssFilter($item->title); ?></title>
		<link><?php echo $item->guid ?></link>
		<category><?php echo wpcRssFilter($item->category) ?></category>
		<guid isPermaLink="true"><?php echo wpcRssFilter($item->guid) ?></guid>
		<!-- dc:creator><?php //the_author() ?></dc:creator -->
		<description>[CDATA[<?php echo wpcRssFilter($item->post) ?>]]</description>
		<pubDate><?php echo wpcRssFilter($item->pubDate) ?></pubDate>
	</item>
<?php
	$contents = ob_get_clean();
   fwrite($fp, $contents);
